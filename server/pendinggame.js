...
class PendingGame {
    ...
    async selectDeck(playerName, deck) {
        ...
        player.chains = 0;
        if(this.gameType === 'chainbound') {
            player.chains = await kip.getChainsForDeckUUID(deck.uuid, playerName);
        }
    ...
    getStartGameDetails() {
        ...
        for(let playerDetails of Object.values(this.players)) {
            const { name, chains, user, ...rest } = playerDetails;
            players[name] = {
                name,
