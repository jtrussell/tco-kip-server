...
export function connectGameSocket(url, name) {
    return (dispatch, getState) => {
        let state = getState();
        let queryString = state.auth.token ? 'token=' + state.auth.token : undefined;
        queryString += `&user=${JSON.stringify(state.account.user)}`;
        ...
        if(window.location.origin.includes('localhost')) {
            ...
            gameSocket = io.connect(window.location.origin, {
                path: '/nodes/node0',
                reconnection: true,
                reconnectionDelay: 1000,
                reconnectionDelayMax: 5000,
